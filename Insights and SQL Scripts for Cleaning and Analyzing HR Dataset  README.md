...
-- If the difference in current time is added to the age minus 100 from the years, all ages come back less than 65 years
...
----------------------------------------------------------------------------------------------------------
  ...
  -- clean date inconsistencies from hire date
...
----- 
	with CTE as (SELECT TERMINATION_DATE,
		CASE WHEN TERMINATION_DATE < "2024-07-06" AND termination_date IS NOT NULL AND TERMINATION_DATE != "" 
	    THEN "TERMINATED" ELSE "EMPLOYED" END AS Employment_Status
	FROM hr_data)
	SELECT COUNT(*),employment_status from CTE
	group by employment_status
...
-- 7. Who is/are the longest-serving employee in the organization?
	...
	SELECT CONCAT(first_name," ",last_name) FULL_NAME,hire_date,termination_date,DATEDIFF(termination_date, hire_date) AS 		 
         Service_Days
	FROM hr_data
